...
def bios(readlake,bin,username,des,password):
    sup2_port = ''
    log = ''
    port = ''
    chassinfo1 = GetAllChassisInfo(dChassis)
    for name,info in  list(chassinfo1.items()):
        print("======name:",name)
        info2 = chassinfo1[name]
        for key,value in list(info2.items()):
            sup2_port = get_supstr(key)
            os.system('telnet' + sup2_port)
            os.system('cd /mnt')
            cmd = "scp -r %s@hw-01:%s/%s ./"%(username,des,bin)
            scp = pexpect.spawn(cmd)
            try:
                i = scp.expect(['password:',pexpect.EOF],timeout = 60)
                if i == 0:
...
def SUP2(readlake,info2):
    for name ,info in list(info2.items()):
        info3 = info2[name]
        if readlake in FM1:
            port = info3[readlake]
            fm = "fm" + port
            os.system('ssh ' + fm)
            cmd1 = "scp -r %s diag@%s:/mnt"%(bin,fm)
            scp1 = pexpect.spawn(cmd1)
            k = scp1.expect([pexpect.EOF],timeout = 5)
            if k == 0:
                scp1.expect()
                os.system('bios_cmd program 0 /mnt/' + bin)
                log = os.system('bios_cmd info')
                os.system('exit')
            ...
            if readlake in SUP1:
                ...
                sup = "sup" + port
                os.system('ssh ' + sup)
                cmd2 = "scp -r %s diag@%s:/mnt"%(bin,sup)
                scp2 = pexpect.spawn(cmd2)
                m = scp2.expect([pexpect.EOF],timeout = 5)
                if m == 0:
                    scp2.expect()
                    os.system('bios_cmd program 0 /mnt/' + bin)
                    log = os.system('bios_cmd info')
...
#    exit()
    ...
    excel_chassis()
    # bios(readlake,bin,username,des,password)
    # chassis_ip()
