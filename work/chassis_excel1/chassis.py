    ...
    try:
        ...
        for line in fileHandle.readlines():
            ...
            if startParser == 1:
                ...
                dChasInfo[name] = INFO1         # merge name into a dict dchasInfo
...
# get the chassis config information
def portfile(filePath):
    LC = {}
    FM = {}
    SUP = {}
    SC = {}
    INFO = None        # store the configure
    try:
...
# merge chassis ip and config into a dict
...
# default path if there is no argv
...
def excel_chassis():
    ...
    try:
        ...
        worksheet.write('B1','IP',bold_format)
        worksheet.write('C1','sup1 CONSOLE',bold_format)
        worksheet.write('D1','sup2 CONSOLE',bold_format)
        worksheet.write('E1','APC',bold_format)
        ...
        ipchassis = telnet('diag-qa-01','root','insieme')
        row = 1
        col = 0
        for name in list(sorted(ipchassis.keys())):
            info = ipchassis[name]
            worksheet.write(row, col, name)
            worksheet.write(row, 1, info['IP0'])
            sup1_str = get_supstr(info['sup1'])
            worksheet.write(row, 2, sup1_str)
            sup2_str = get_supstr(info['sup2'])
            worksheet.write(row, 3, sup2_str)
            worksheet.write(row, 4, info['APC'])
...
def telnet(Host,username,password):
    ...
    yes_day_nyr = yes_day.strftime("%m-%d-%Y")
    for name,info in list(chasinfo.items()):
        info = chasinfo[name]
        sup1_port = get_supstr(info['sup1'])
        eth = eth_num(info['SUP'])
        print("=======",name,"=======")
        info['IP0'] = "NULL"
        eth = "ifconfig eth"
        cmd = "grep \"ifconfig eth"       # + str(ethPort)a

        cmd = cmd + "\" -A 2 /vol/diag/logs/"

        cmd = cmd + name + "/" + yes_day_nyr + "*.1"
        child = pexpect.spawn('ssh %s@%s "%s"' %(username,Host, cmd))
        res = child.expect(['password:',pexpect.EOF],timeout=5)
        try:
            if res == 0:
                ...
                ret =  regExp.search(r.decode('utf-8'))
                if ret:
                    ip_str = ret.group(0)
                    print("1",ip_str)
                    info['IP0'] = ip_str    # add ip into dict dchassis
...
if __name__ == '__main__':
    ...
    # telnet('diag-qa-01','root','insieme')
    # excel_chassis()
